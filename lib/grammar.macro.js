...
import { e, eat, eatMatch } from '@bablr/helpers/grammar';
...
export const grammar = class CCommentsGrammar {
  *Trivia() {
    while (yield eatMatch(m`#: <*Space:Space />`) || (yield eatMatch(m`<Comment />`)));
  }

  *Comment() {
    yield eat(m`<Any />`, e([m`<BlockComment '/*' />`, m`<LineComment '//' />`]));
  }
